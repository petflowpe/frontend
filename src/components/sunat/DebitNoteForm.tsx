...
const MOTIVOS_ND = [
  ...
  { code: '03', name: 'Penalidades' },
  { code: '10', name: 'Otros' },
  { code: '11', name: 'Ajustes de exportación' },
];

export function DebitNoteForm({ open, onOpenChange, onSuccess, companyId = 1, branchId = 1 }: { open: boolean; onOpenChange: (v: boolean) => void; onSuccess?: () => void; companyId?: number; branchId?: number }) {
  ...
  const submit = async () => {
    ...
    try {
      ...
      await apiClient.post('/debit-notes', {
        ...
        des_motivo: desMotivo || (MOTIVOS_ND.find(m => m.code === codMotivo)?.name ?? 'Otros'),
        client: clientPayload,
        detalles: [{
          codigo: codigo || 'ND-001',
          descripcion,
          unidad: 'NIU',
      ...
      toast.error(e.message || 'Error al crear nota de débito');
    } finally {
      setSubmitting(false);
    }
  };
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        ...
        <DialogFooter>
          ...
          <Button onClick={submit} disabled={submitting}>{submitting ? 'Guardando...' : 'Crear nota de débito'}</Button>
        </DialogFooter>
      </DialogContent>
    </Dialog>
  );
